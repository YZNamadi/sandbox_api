...
export class MocksService {
  ...
  async cacheMockResponse(sandboxId: string, path: string, method: string, response: any): Promise<void> {
    await this.redis.set(`sandbox:${sandboxId}:mock:${method}:${path}`, JSON.stringify(response));
  }

  async getCachedMockResponse(sandboxId: string, path: string, method: string): Promise<any | null> {
  ...
  async saveCustomMock(sandboxId: string, path: string, method: string, response: any, isRandomized = false, delayMs?: number) {
  ...
  async getMockResponse(sandboxId: string, path: string, method: string) {
