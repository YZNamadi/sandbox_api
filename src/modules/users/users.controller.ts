...
export class UsersController {
  ...
  @ApiBody({ schema: { properties: { email: { type: 'string' }, name: { type: 'string' }, roleName: { type: 'string' } } } })
  @ApiResponse({ status: 201, description: 'User invited.' })
  async inviteUser(@Req() req: Request, @Body() body: { email: string; name: string; roleName: string }) {
    const user = req.user as JwtUser;
    const teamId = user.teamId;
    if (!teamId) throw new Error('Missing teamId in JWT payload');
    return this.usersService.inviteUser(teamId, body.email, body.name, body.roleName);
  }
  ...
  async changeUserRole(@Req() req: Request, @Param('userId') userId: string, @Body() body: { roleName: string }) {
    const user = req.user as JwtUser;
    const teamId = user.teamId;
    if (!teamId) throw new Error('Missing teamId in JWT payload');
